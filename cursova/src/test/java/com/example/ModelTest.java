...
public class ModelTest {
	...
	public void restTest() {
		...
		Producer producer = new Producer(
			"Producer", FunRand.getErlang(16, 32),1);

		SingleChannelSMO smo1 = new SingleChannelSMO(
			"Zero",1000,() -> 0.0,2);

		SingleChannelSMO smo2 = new SingleChannelSMO(
			"SMO",FunRand.getExponential(12),3);

		SingleChannelSMO rest = new SingleChannelSMO(
			"Rest",FunRand.getFixed(5),4); 
		rest.setDoneStatus();
		...
		connection2.addNext(smo2, () -> rest.getChannelStatus().isDone());

		restcConnection.addNext(rest, () -> smo2.getChannelStatus().isDone());
	...
	public void modelTest() {
		...
		System.out.println("mean_loader_utilization: " + mean_loader_utilization);
		System.out.println("mean_truck_utilization: " + mean_truck_utilization);

		model.getStats().forEach((stats) -> System.out.println(stats));
	}

	@Test
	public void modModelTest() {
		ArrayList<SingleChannelSMO> list = new ArrayList<>();
		...
		SingleChannelSMO truck5 = new SingleChannelSMO(
			"Truck5", FunRand.getCombined(List.of(
				FunRand.getNotNullNorm(22, 10), 
				FunRand.getUniform(2, 8)
		)), 4);
		...
		SingleChannelSMO rest15 = new SingleChannelSMO(
			"Rest15", FunRand.getNotNullNorm(18, 10), 6);
		rest15.setDoneStatus();
		...
		connection2.addNext(loader1, () -> {
			...
			if (truck4.getChannelStatus().isReady() && rest14.getChannelStatus().isDone()) 
				countB++;
			if (truck5.getChannelStatus().isReady() && rest15.getChannelStatus().isDone()) 
				countB++;
		...
		connection3.addNext(truck5, () -> {
			return rest15.getChannelStatus().isDone();
		});
		...
		Connection restCon15 = new Connection();
		rest15.setNext(restCon15);
		restCon15.addNext(rest15, () -> truck5.getChannelStatus().isDone());
		...
		var truck3St = truck3.getStats();
		var truck4St = truck4.getStats();
		var truck5St = truck5.getStats();

		var rest11St = rest11.getStats();
		var rest12St = rest12.getStats();
		var rest13St = rest13.getStats();
		var rest14St = rest14.getStats();
		var rest15St = rest15.getStats();
		...
		double mean_truck_utilization = 
			((truck1St.getBusyTime() + truck2St.getBusyTime() + truck3St.getBusyTime() + truck4St.getBusyTime() + truck5St.getBusyTime()) + 
			(rest11St.getBusyTime() + rest12St.getBusyTime() + rest13St.getBusyTime() + rest14St.getBusyTime() + rest15St.getBusyTime()) +
			(loader1St.getBusyTime() + loader2St.getBusyTime())) / 
			5 / (truck1St.getTotalSimTime());
